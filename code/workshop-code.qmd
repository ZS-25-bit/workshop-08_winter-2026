...
---
[deployed html url](https://zs-25-bit.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
# add in new data frame
drought_exp_clean <- drought_exp |> 
  # cleaned up names with underscores and lowercase 
  clean_names() |>  
...
ggplot(data = drought_exp_clean, # use the annual_visits data frame
       aes(x = sla, # the x-axis 
           y = total_g, # the y-axis 
           )) + # color all geometries by species name
  geom_point() + # adding points
  labs(x = "Specific Leaf Area cm\U00B2(g)", # relabelling the x-axis
       y = "Total Mass(g)", # and the y-axis
       title = "As SLA increases, Total Mass Increases", # adding a title
...
ggplot(data = drought_exp_clean, # starting data frame
  ...
  geom_jitter(height = 0, # making sure points don't move along y-axis
              width = 0.3) + # narrowing width of jitter 
  ...
  labs(x = "Water Treatment",
       y = "Total mass(g)",
       title = " Well Waterted Plants tend to have a higher mass") +
...
ggplot(data = drought_exp_clean, # starting data frame
       aes(x = reorder(species_name, -total_g), # x-axis
           y = total_g, # y-axis
  ...
  scale_color_manual(values = c(
   ...
   "Eschscholzia californica" = enccal_col,
...
Arrange by decreasing AIC. 
Best model is model4: the one that includes water treatment and species.
...
Residuals look homoscedastic(have constant varaiance) based on Residual vs. Fitted plot and SCale-Location plot. 
NO outliters infludencing model estimates of coeffiecints 
...
Estimated mass for drought stress lotus scoparius 0.055 g +/- 0.02(SE)


**What does `water_treatmentWell watered` represent?**
This is the model prediction for the difference in lotus scorparis mass between the well watered and drought strees treatments. Y=The model predicts the difference is -0.12 +/- 0.02(SE) g. 

**What does `species_namePenstemon centranthifolius` represent?**  
This si the model prediction fo t the difference between Penstremon centranthifollius mass in the well watered treatment and Lotus scoparoius mass in the well watered treatment. The model predicts that this difference is 0.05 +/- 0.03(SE) g. 


**What does `species_nameEschscholzia californica` represent?**
This is the model prediction for the difference between Eschscholzia californica mass in the well watered treatment and lotus scoparius mass in the well watered treatment. The model predicts that is difference is 0.23 =/- 0.03(SE) g. 
...
Water treatment influcence plant mass. 
Species influence plant mass. 
